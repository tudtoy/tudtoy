<section class="own own-collectors">
    <div class="container own__container">
        ...
        <?php if ($args['background_image']) : ?>
            <img class="own__card-bg own-collectors__card-bg<? if ($args['background_image_is_full']) echo ' own-collectors__card-bg--full'; ?>" src="<?=$args['background_image']['url']?>">
        <?php endif; ?>
        ...
        <?php
            global $product;
        ?>
        <div class="own__content own-collectors__content own-collectors__content-second">
            <span class="text_body product-price__value"><?=$product->get_price_html()?></span>
        </div>
        <div class="own__content own-collectors__content own-collectors__content-second">
        ...
        <? 
            ...
            if (isset($args['cards']) && is_countable($args['cards'])) {
                $cardCount = count($args['cards']);
            }

            $navClass = $cardCount < 4 ? 'own__nav_lock' : '';
        ?>
